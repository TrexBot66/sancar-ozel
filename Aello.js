...
client.on('guildMemberAdd', async member => {
    let tag = await client.users.fetch('539108436722319400')
    let gün = moment.duration(Date.now() - member.user.createdTimestamp).format("D");
    if (gün < Config.memberAdd.guvenliMin) { knt = 'Tehlikeli! <a:uyari:941802557133713448>' } else { knt = 'Güvenli! <a:onay:941802574984650852>' }
    if (knt === 'Tehlikeli!') {
        ...
        if (cache.get(member.id) == 2) {
            ...
            member.ban({ reason: 'HydroniX Token Koruması.' });
    ...
    let hydronix = Config.memberAdd.hydronix.replace('{user}', member)
    let desc = Config.memberAdd.hgMesaji.replace('{user}', member)
        ...
        .replace('{kontrol}', knt)
        .replace('{old}', gün)
    client.channels.cache.get(Config.Channels.Welcome).send({ content: `** ${hydronix} \n\n Kayıt Odasına **${Config.Roles.welcomeRegOfficer}`,
        embeds: [new Discord.MessageEmbed().setColor('6e88c8').setFooter(`Dev By ${tag}`).setDescription(desc).setThumbnail(member.user.avatarURL({ size: 2048, dynamic: true })).setTitle(`Aramıza Yeni Birisi Katıldı`)]
    })
});