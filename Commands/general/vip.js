...
module.exports = {
    ...
    run(message, args, client) {
        ...
        if (!member) return message.reply('Lütfen vip olacak üyeyi belirtiniz.')
        let isim = args[1];
        let yas = args[2];
        
        member.roles.add(Config.Roles.vip).catch(e => { });

        message.channel.send({ embeds: [new Discord.MessageEmbed().setFooter(message.guild.name).setDescription(`**Kullanıcıya başarılı bir şekilde <@&${Config.Roles.vip}> permi verildi.**`)] })
    }}