...
module.exports = {
    ...
    run(message, args, client) {
        ...
        if (!member) return message.reply('Lütfen garson olacak üyeyi belirtiniz.')
        let isim = args[1];
        let yas = args[2];
        
        member.roles.add(Config.Roles.garson).catch(e => { });
        ...
        message.channel.send({ embeds: [new Discord.MessageEmbed().setFooter(message.guild.name).setDescription(`**Kullanıcıya <@&${Config.Roles.garson}> , <@&${Config.Roles.sancarpersonel}> ve <@&${Config.Roles.sancarplus}> rollerini verdim \nKullanıcıdan <@&${Config.Roles.misafir}> rolünü aldım.**`)] })
    }}